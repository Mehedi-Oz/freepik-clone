...
try {
  ...
  if (!$slis) {
    ...
    if (ENVIRONMENT === 'production') {
      echo json_encode([
        'error' => 'Unable to retrieve results. Please try again.',
        'html' => '',
        'total' => 0,
      ]);
    } else {
      echo json_encode([
        'error' => 'Query failed: ' . mysqli_error($dbcon),
        'html' => '',
        'total' => 0,
      ]);
    }
  ...
  if ($hasResults) {
    ...
    if ($slis instanceof ArrayIterator || is_array($slis)) {
      ...
      foreach ($slis as $list) {
        ...
        $html .=
          '<div class="image-card relative group mb-4" data-image="' .
          $imageId .
          '">
                      <img src="https://cdn.pixahunt.com/' .
          $thumbImg .
          '"
    ...
    $html =
      '<div class="w-full text-center text-white py-20 text-base col-span-full">No results found. Try adjusting your filters or search terms.</div>';
  }
  ...
  if (ENVIRONMENT === 'production') {
    ...
    error_log(
      'API Error: ' . $e->getMessage() . ' in ' . $e->getFile() . ' on line ' . $e->getLine(),
    );
